...
class Grid:
    ...
    def __init__(
        ...
        print(
            f"Total grid size is {total_grid_size}, meaning {configs_per_hp} values per hyperparameter."
        )
        self.hp_values = {}
        for hp in configspace:
            if isinstance(configspace[hp], CategoricalHyperparameter):
                ...
                self.hp_values[hp] = [
                    configspace[hp].choices[min(int(v), len(configspace[hp].choices) - 1)]
                    for v in self.hp_values[hp]
                ]
            else:
                self.hp_values[hp] = np.linspace(
                    configspace[hp].lower, configspace[hp].upper, configs_per_hp
                )
