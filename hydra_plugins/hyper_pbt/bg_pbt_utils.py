        ...
        while n_step <= step:
            ...
            with torch.no_grad():
                x_tensor = torch.maximum(
                    torch.minimum(x_tensor, torch.tensor(ub).to(x_tensor.dtype)),
                    torch.tensor(lb).to(x_tensor.dtype),
                )
            n_step += 1
...
def get_start_point(
    ...
    for i in range(new_config_array.shape[0]):
        ...
        if isinstance(cs[param_name], CSH.CategoricalHyperparameter):
            ...
            type(cs[param_name])
            in [CSH.UniformIntegerHyperparameter, CSH.NormalIntegerHyperparameter]
...
def sample_discrete_neighbour(cs: CS.ConfigurationSpace, x, frozen_dims: list[int] | None = None):
    ...
    while config_pert[selected_dim] is None or (
        frozen_dims is not None and index_in_array in frozen_dims
    ):
        selected_dim = str(rng.choice(cat_dims + int_dims, 1)[0])
        index_in_array = cs.index_of(selected_dim)
        ...
        if selected_dim in ["NAS_policy_num_layers", "NAS_q_num_layers"]:
            ...
            candidates = list(
                range(
                    max(
                        lb,
                        min(config[selected_dim] - 1, round(config[selected_dim] * 0.8)),
                    ),
                    min(
                        ub,
                        max(round(config[selected_dim] * 1.2), config[selected_dim] + 1),
                    )
                    + 1,
                )
            )
        ...
        while n_step <= step:
            ...
            for _ in range(interval):
                ...
                with torch.no_grad():
                    ...
                    x_tensor[cont_dims] = torch.maximum(
                        torch.minimum(x_tensor[cont_dims], torch.tensor(ub[cont_dims])),
                        torch.tensor(lb[cont_dims]),
                    ).to(x_tensor.dtype)
            ...
            if len(cat_dims) + len(int_dims) > 0:
                for _j in range(interval):
                    neighbours = [
                        sample_discrete_neighbour(cs, x, frozen_dims=frozen_dims)
                        for _ in range(10)
                    ]
                    ...
                    acq_x = f(x).detach().numpy()
                    acq_neighbour = np.array([f(n).detach().numpy() for n in neighbours]).astype(
                        float
                    )
...
class CasmoKernel(gpytorch.kernels.Kernel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.categorical_kern = ExpCategoricalOverlap(
            ard_num_dims=len(self.cat_dims) if ard else None,
    ...
    def forward(
        ...
        **params,
        ...
        assert (
            x1.shape[1] >= self.dim
        ), f"Dimension mismatch! Expected = {self.dim} but got {x1.shape[1]}"
        ...
        if t2 is not None and self.time_kernel is not None:
            assert (
                t2.shape[0] == x2.shape[0]
            ), "Dimension mismatch between x2 and its timestep vector t2!"
    ...
    def forward(
        ...
        with torch.no_grad():  # discrete kernels are not differentiable. Make is explicit as such
            ...
            def rbf(d, ard):
                if ard:
                    return torch.exp(
                        torch.sum(d * self.lengthscale, dim=-1) / torch.sum(self.lengthscale)
                    )
                else:  # noqa: RET505
...
class Casmo4RL(HyperparameterOptimizer):
    ...
    def restart(self):
        ...
        self.X_init = np.array(
            [self.env.sample_configuration().get_array() for _ in range(self.n_init)]
        )
    ...
    def run(self):
        ...
        while self.cur_iters < self.max_iters:
            ...
            suggested_configs = [
                CS.Configuration(self.env.config_space, vector=array)
                for array in suggested_config_arrays
            ]
            ...
            if len(self.y):
                ...
                if self.cur_iters % self.log_interval == 0 and self.log_dir is not None:
                    logging.info(
                        f'Saving intermediate results to {Path(self.log_dir)/"stats.pkl"}'
                    )
    ...
    def _obj_func_handle(
        ...
        return [
            -get_reward_from_trajectory(np.array(t["y"]), use_last_fraction=self.use_reward)
        ...
        if batch_size == 1:
            ...
            if self.use_standard_gp:
                ...
                X_next, acq_next = interleaved_search(
                    ...
                    cont_int_lengthscales=gp.covar_module.base_kernel.lengthscale.cpu()
                    .detach()
                    .numpy()
                    .ravel(),
                    n_restart=self.n_restart,
