...
class PB2(PBT):
    ...
    def get_continuous(self, config, performance, X, y):
        ...
        if len(self.current) == 0:
            ...
            padding = np.array(
                [
                    [len(self.config_history) // self.population_size, performance]
                    for _ in range(current_use.shape[0])
                ]
            )
            max_perf = min(*self.performance_history[-self.population_size :], performance)
            padding = normalize(
                padding, [len(self.config_history) // self.population_size, max_perf]
            )
            ...
            if self.categorical_mutation == "mix" and len(self.categorical_hps) > 0:
                cat_locs = [
                    len(self.X[0]) - x - 1 for x in reversed(range(len(self.cat_values[0])))
                ]
        ...
        if self.categorical_mutation == "mult":
            ...
            for i in reversed(range(1000 // self.population_size)):
                for j in range(self.population_size):
                    ...
                    ys.append(
                        self.performance_history[-(i - 1) * j] - self.performance_history[-i * j]
                    )
                    tps.append([t, p])
            ...
            self.ys = np.array(ys)
            self.fixed = normalize(
                tps,
                [
                    len(self.performance_history // self.population_size),
                    max(performances),
                ],
            )
            self.cat_current = []
            ...
            performance_data, config_data = deepcopy(self.performance_history), deepcopy(
                self.config_history
            )
            ...
            for i in range(self.population_size):
                for j in range(iterations_run):
                    cvs = [
                        v
                        for v, n in zip(
                            config_data[i * j].values(),
                            config_data[i * j].keys(),
                            strict=False,
                        )
                        if n in self.categorical_hps
                    ]
    ...
    def fit_model(self, performances, configs):
        ...
        if self.categorical_mutation == "mix" and len(self.categorical_hps):
            ...
            kernel = TVSquaredExp(
                input_dim=self.X.shape[1], variance=1.0, lengthscale=1.0, epsilon=0.1
            )
