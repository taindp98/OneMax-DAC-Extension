...
class TVSquaredExp(GPy.kern.Kern):
    ...
    def K(self, X, X2):
        ...
        RBF = self.variance * np.exp(
            -np.square(metrics_pairwise.euclidean_distances(X, X2)) / self.lengthscale
        )
...
class TVMixtureViaSumAndProduct(GPy.kern.Kern):
    ...
    def update_gradients_full(self, dl_kl, x, x2):
        ...
        dl = -(
            metrics_pairwise.euclidean_distances(X_cont, X2_cont) ** 2
            * self.variance_1
            * np.exp(-dist2)
        ) * self.lengthscale ** (-2)
...
def timevarying_compute_prob_dist_and_draw_hts(weights, gamma, batch_size, omega, pending_actions):
    ...
    if batch_size < C:
        ...
        myselection = rng.choice(
            len(probabilityDistribution), batch_size, p=probabilityDistribution
        )
...
def exp3_get_cat(row, data, num_rounds, index):
    ...
    if batch_size < numActions:
        gamma = math.sqrt(
            numActions
            * np.log(numActions / batch_size)
            / ((np.e - 1) * batch_size * (num_rounds / 10))
        )
    else:
        gamma = 0.2
...
def with_proba(epsilon):
    ...
    assert (
        0 <= epsilon <= 1
    ), f"""Error: for 'with_proba(epsilon)', epsilon = {epsilon:.3g}
...
def DepRound(weights_p, k=1):
    ...
    assert (
        len(subset) == k
    ), f"""Error: DepRound({weights_p}, {k}) is supposed
...
def DepRound2(weights_p, k=1):
    ...
    assert (
        len(subset) == k
    ), f"""Error: DepRound({weights_p}, {k}) is
