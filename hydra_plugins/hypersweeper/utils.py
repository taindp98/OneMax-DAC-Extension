...
def convert_to_configuration(x: pd.Series, configspace: ConfigurationSpace) -> Configuration:
    ...
    x = dict(x)
    hp_config = {
        k: maybe_convert_types(k, v, configspace=configspace)
        for k, v in x.items()
        if k.startswith("hp_config")
    }
    return Configuration(configuration_space=configspace, values=hp_config)


def read_warmstart_data(
    warmstart_filename: str, search_space: ConfigurationSpace
    ...
    infos = [
        Info(config=c, budget=b, seed=s) for c, b, s in zip(configs, budgets, seeds, strict=False)
    ]
    # the cost in hypersweeper is the runtime of the configuration
    # cost is not tracked for initial design
    results = [
        Result(info=i, performance=p, cost=0.0)
        for i, p in zip(infos, logged_performances, strict=False)
    ]
...
if __name__ == "__main__":
    search_space = OmegaConf.load(
        "/home/numina/Documents/repos/ARLBench/runscripts/configs/search_space/dqn_cc.yaml"
    )
    read_warmstart_data(
