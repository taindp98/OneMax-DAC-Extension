...
class HypersweeperSweeper:
    ...
    def __init__(
        ...
        if warmstart_file:
            self.warmstart_data = read_warmstart_data(
                warmstart_filename=warmstart_file, search_space=self.configspace
            )
    ...
    def run_configs(self, infos):
        ...
        for i in range(len(infos)):
            ...
            if self.slurm and self.max_budget is not None:
                ...
                optimized_timeout = (
                    self.slurm_timeout * 1 / (self.max_budget // infos[i].budget)
            ...
            if self.seeds:
                for s in self.seeds:
                    ...
                    load_path = (
                        ...
                        / f"{infos[i].load_path!s}_s{s}{self.checkpoint_path_typing}"
                    )
                    ...
                    job_overrides = tuple(self.global_overrides) + tuple(
                        f"{name}={val}"
                        for name, val in zip(
                            [*names, self.seed_keyword], [*local_values, s], strict=True
                        )
                    )
                ...
                load_path = (
                    Path(self.checkpoint_dir)
                    / f"{infos[i].load_path!s}{self.checkpoint_path_typing}"
                )
                save_path = self.get_save_path(i)
        ...
        if self.seeds and self.deterministic:
            ...
            for config_idx in range(len(overrides) // n_seeds):
                performances.append(
                    [
                        res[config_idx * n_seeds + seed_idx].return_value
    ...
    def get_save_path(self, config_id, seed=None):
        ...
        if self.seeds:
            ...
            save_path = (
                Path(self.checkpoint_dir)
                / f"iteration_{self.iteration}_id_{config_id}{self.checkpoint_path_typing}"
            )
        else:
            save_path = (
                Path(self.checkpoint_dir)
    ...
    def _write_csv(self, data: dict, filename: str) -> None:
        ...
        if "config_id" not in dataframe.columns:
            dataframes_to_concat += [
                pd.DataFrame(np.arange(len(dataframe)), columns=["config_id"])
            ]
        ...
        for i in range(len(configs)):
            ...
            if self.seeds:
                ...
                for seed_idx, seed in enumerate(self.seeds):
                    self.history[f"performance_{self.seed_keyword}_{seed}"].append(
                        performances[i][seed_idx]
                    )
            else:
