...
def run_random(results_path, benchmark_name, num_episodes, seeds, fixed):
    ...
    for s in seeds:
        ...
        logger = Logger(experiment_name=experiment_name, output_path=results_path / benchmark_name)
        env = bench.get_benchmark(seed=s)
        env = PerformanceTrackingWrapper(env, logger=logger.add_module(PerformanceTrackingWrapper))
...
def main(args):
    ...
    possible_actions = ", ".join(
        [f"{benchmark} : {actions}" for benchmark, actions in shortened_possible_actions.items()]
    )
    parser.add_argument(
        "--actions",
        nargs="+",
