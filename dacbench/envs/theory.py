    ...
    def crossover(
        ...
        for l in ls:
            ...
            if (obj != self.fitness) and (obj != xprime.fitness):
                n_evals += 1
            elif (not np.array_equal(xprime.data[locs_xprime], self.data[locs_xprime])) and (
                not np.array_equal(self.data[locs_x], xprime.data[locs_x])
            ):
                n_evals += 1
    ...
    def step(self, actions, **kwargs):
        ...
        if stop is False:
            ...
            if self.reward_choice == "imp_div_evals":
                ...
                reward = ((self.x.fitness - fitness_before_update - n_evals) / self.n) + kwargs[
                    "shift"
                ]
            self.log_reward.append(reward)
    ...
    def step(self, actions, **kwargs):
        ...
        if stop is False:
            ...
            if len(actions) == 4:
                ...
                raise ValueError(
                    "Invalid number of actions provided. Expected 1 or 4, got %d." % len(actions)
                )
            ## clip rate to [0,1]
            mutation_rate = np.clip(mutation_rate, 0, 1)
            crossover_rate = np.clip(crossover_rate, 0, 1)
...
class OLLGAFactTheoryEnvDiscrete(OLLGAFactTheoryEnv):
    ...
    def __init__(self, config, test_env=False):
        ...
        assert self.action_space.n == len(
            config["action_choices"][0][0]
        ), "Error: action space's size (%d) must be equal to the len(action_choices) (%d)" % (
            self.action_space.n,
            len(config["action_choices"][0][0]),
        )
        self.discrete_action = True
        self.action_choices = config["action_choices"]
...
class OLLGATheoryPPOEnvDiscrete(OLLGATheoryPPOEnv):
    ...
    def step(self, actions, **kwargs):
        ...
        if len(actions) == 4:
            ...
            raise ValueError("Invalid action length: expected 1 or 4, got {}".format(len(actions)))
