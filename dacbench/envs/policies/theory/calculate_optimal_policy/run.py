...
def break_points_to_policy(n: int, portfolio: List[int], break_points: List[int]) -> List[int]:
    """
...
def save_policy(n: int, portfolio: List[int], break_points: List[int], policy_file: str):
    """
    ...
    """
    policy_str = (
        f"{n};{','.join([str(r) for r in portfolio])};{','.join([str(p) for p in break_points])}"
    )
...
def determine_breaking_point(larger: int, smaller: int, n: int) -> int:
    ...
    breaking_point = 0
    for i in range(1, n):
        if improvement_probability(n, larger, i) < improvement_probability(n, smaller, i):
            break
        breaking_point = i
