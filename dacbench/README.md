...
class OLLGAFactL1TheoryEnv(OLLGATheoryEnv):
    """
    Environment for single-parameter control in (1+(lbd,lbd))-GA using DDQN
    The action space representation is in factored structure
    """

class OLLGATheoryPPOEnv(OLLGATheoryEnv):
    """
    Environment for single-parameter control in (1+(lbd,lbd))-GA using PPO
    The action space representation is in factored structure
    """
```
...
class OLLGAFactL1TheoryBenchmark(OLLGATheoryBenchmark):
    """
    Benchmark for single-parameter control in (1+(lbd,lbd))-GA using DDQN
    The action space representation is in factored structure
    """

class OLLGATheoryPPOBenchmark(OLLGATheoryBenchmark):
    """
    Benchmark for single-parameter control in (1+(lbd,lbd))-GA using PPO
    The action space representation is in factored structure
    """
```
