...
class AbstractBenchmark:
    ...
    def serialize_config(self):
        ...
        for k in self.config.keys():
            if isinstance(self.config[k], np.ndarray) or isinstance(self.config[k], list):
    ...
    def from_json(cls, json_config):
        ...
        if "config_space" in config.keys():
            ...
            if "name" in config.config_space:
                configuration_space = ConfigurationSpace(name=config.config_space["name"])
            else:
    ...
    def load_config(self, config: "objdict"):
        ...
        if "observation_space" in self.config:
            self.config["observation_space"] = self.list_to_space(self.config["observation_space"])
        ...
        if "action_space" in self.config:
            self.config["action_space"] = self.list_to_space(self.config["action_space"])
