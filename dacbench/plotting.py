...
def add_multi_level_ticks(
    ...
    for ax in grid.axes.flat:
        ...
        new_labels = [f"{epoch}:{step}" if epoch is not None else "" for epoch, step in new_labels]
...
def plot_performance(data, title=None, x_label=None, y_label=None, **kwargs) -> sns.FacetGrid:
    """
...
def plot_step_time(
    ...
    **args,
    ...
    if interval > 1:
        ...
        y_label = f"Mean per duration per {interval} steps" if y_label is None else y_label
...
def plot_episode_time(data, title=None, x_label=None, y_label=None, **kargs) -> sns.FacetGrid:
    """
...
def plot_action(
    ...
    **kargs,
    ...
    """
    return plot_space(data, "action", show_global_step, interval, title, x_label, y_label, **kargs)
...
def plot_state(
    ...
    **kargs,
    ...
    """
    return plot_space(data, "state", show_global_step, interval, title, x_label, y_label, **kargs)
...
def plot_space(
    ...
    **args,
    ...
    space_entries = list(filter(lambda col: col.startswith(space_column_name), data.columns))
    ...
    if number_of_space_entries > 1:
        ...
        data = pd.wide_to_long(
            ...
            sep="_",
            i=["episode", "step", "instance"] + (["seed"] if "seed" in data.columns else []),
            j="i",
            suffix=".*",
        ).reset_index()
    ...
    if interval > 1:
        ...
        group_columns = list(data.columns.drop(x_label_columns + [x_column, space_column_name]))
        data = data.groupby(group_columns).agg({x_column: "min", space_column_name: "mean"})
        y_label = f"Mean {y_label_name} per {interval} steps" if y_label is None else y_label
