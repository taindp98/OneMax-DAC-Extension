...
def flatten_log_entry(log_entry: Dict) -> List[Dict]:
    ...
    for value_name, value_dict in dict_entries:
        current_rows = (
            dict(top_level_entries + [("value", value), ("time", time), ("name", value_name)])
...
def list_to_tuple(list_: List) -> Tuple:
    ...
    """
    return tuple(list_to_tuple(item) if isinstance(item, list) else item for item in list_)
...
class ModuleLogger(AbstractLogger):
    ...
    def __end_step(self):
        if self.current_step:
            ...
            self.buffer.append(json.dumps(self.current_step, default=self.__json_default))
        self.current_step = self.__init_dict()
    ...
    def next_step(self):
        """Call at the end of the step. Updates the internal state and dumps the information of the last step into a json."""
        self.__end_step()
        if self.step_write_frequency is not None and self.step % self.step_write_frequency == 0:
            self.write()
        self.step += 1
    ...
    def log(self, key: str, value: Union[Dict, List, Tuple, str, int, float, bool]) -> None:
        """
        Writes value to list of values and save the current time for key.
    ...
    def __space_dict(key: str, value, space_info):
        ...
        if isinstance(value, Number):
            ...
            key_suffix = enumerate(value) if space_info is None else zip(space_info, value)
            data = {f"{key}_{suffix}": x for suffix, x in key_suffix}

        elif isinstance(value, dict):
            key_suffix = value.items() if space_info is None else zip(space_info, value.values())
            dicts = (
